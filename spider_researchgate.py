        ...
        if response.status_code == 200:
            ...
            for article in articles:
                ...
                for a_url,author_name in zip(author_urls,author_names):
                    if author_name not in authors_all.keys() or \
                        (author_name in authors_all.keys() and "publications" not in authors_all[author_name].keys()):
                        author_info = get_author_info(a_url)
                        if author_info is None:
                            author_info = {}
                        authors_all[author_name] = author_info
                        authors_all[author_name]["articles"] = [article_title]
                        
                    else:
                        if article_title not in authors_all[author_name]["articles"]:
                            authors_all[author_name]["articles"].append(article_title)
                    authors_all[author_name]["url"] = a_url
                    authors_all[author_name]["name"] = author_name
...
results = readinfo("results.json")
titles = readinfo("titles.json")

authors_all = readinfo("authors.json")
for idx,title in tqdm(enumerate(titles)):
    ...
    try:
        ...
        writeinfo("results.json",results)
        writeinfo("authors.json",authors_all)
    except:
        results[str(idx)] = {}